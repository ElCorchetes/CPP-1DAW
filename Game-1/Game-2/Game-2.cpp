...
void spells() {
    ...
    if (in == "1" && mp >= 7) {
        ...
        armor = 5;
        spell = 1;
        cout << "You invoke a magic shield\n";
    }
    else if (in == "2" && mp >= 10) {
        mp -= 10;
        enemyLife -= 4;
        spell = 2;
        cout << "You stun Cerbero: -4 hp\n";
    }
    else if (in == "3" && mp >= 12) {
        mp -= 12;
        enemyLife -= 17;
        cout << "You use Hydro Pump: -17 hp\n";
    }
    else {
        cout << "Not enough MP\n";
    }
...
void inventory() {
    ...
    if (in == "1" && hpPotion > 0) {
        ...
        cout << "You use heal, now have: " << life << " hp\n";
    }
        ...
        cout << "You use mana potion, now have: " << mp << " mp\n";
    }
    else {
...
void enemyAttackHero() {
    ...
    if (spell == 1) {
        if ((damage - armor) > 0) {
            life -= (damage - armor);
            cout << "Cerbero bites you: -" << (damage - armor) << " hp\n";
        }
        else {
...
int main() {
    ...
    //cout << "A dark cave, the echo of your footsteps reverberates. Suddenly\n";
    //for (int i = 0; i < 2145000000; i++) {}
    //cout << "Unknown: WHAT ARE YOU DOING IN MY CAVE?\n";
    //for (int i = 0; i < 2145000000; i++) {}
    //cout << "You: I didn’t come here looking for trouble\n";
    //for (int i = 0; i < 2145000000; i++) {}
    //cout << "Cerbero: Hahaha! I am Cerbero, the guardian of this place\n";
    //for (int i = 0; i < 2145000000; i++) {}
    //cout << "You: What is so funny?\n";
    //for (int i = 0; i < 2145000000; i++) {}
    //cout << "Cerbero: Very well, human. Let the dance begin!\n";
    //for (int i = 0; i < 2145000000; i++) {}
    ...
    if (enemyLife <= 0) {
        cout << "Cerbero: HOW IS THIS POSSIBLE!\n";
        for (int i = 0; i < 2145000000; i++) {}
        cout << "You WIN!\n";
    }
    else {
        cout << "You LOST! Cerbero: Your bones will be perfect for my collection!\n";
        for (int i = 0; i < 2145000000; i++) {}
    }
